...
static OSStatus recordingCallback(void *inRefCon,
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if(audioController->sendType == 0) {
            [audioController.delegate sendVoice:data];
        } else {
            [audioController.delegate sendVoiceForKws:data];
        }
...
- (void)setupRecoder {
    NSURL *soundFile = [NSURL fileURLWithPath:@"/dev/null"];
    NSDictionary *recoderSettings = @{AVFormatIDKey: @(kAudioFormatAppleLossless),
    ...
    @try {
        NSError *anErr = nil;
        self->recoder = [[AVAudioRecorder alloc] initWithURL:soundFile settings:recoderSettings error:&anErr];
        self->recoder.delegate = self;
        [self->recoder setMeteringEnabled:YES];
        [self->recoder prepareToRecord];
    } @catch (NSException *exception) {
        self->recoder = nil;
        NSLog(@"[InsideSDK] Recorder error { %@ }", [exception description]);
    } @finally {
        //code
    }
}
- (void)updateAudioMeter:(NSTimer *)timer {
    if (self->recoder != nil) {
        ...
        if (self->recoder.isRecording) {
            ...
            CGFloat peak = [self->recoder peakPowerForChannel:0];
            //[self.delegate grpcManagerEventHandler:GRPC_STATUS_RMS_CHANGED response:@{@"peak": [NSNumber numberWithFloat:peak]}];
            [self.delegate sendRms:[NSNumber numberWithFloat:peak]];
        }
        
        else if (self->timerCouner == 50) {
            [self stopRecorder];
        }
    }
}
...
- (void)startRecorder {
    ...
    [self->recoder record];
    self->timerCouner = 0;
    self->meterTimer = [NSTimer scheduledTimerWithTimeInterval:0.009 target:self selector:@selector(updateAudioMeter:) userInfo:nil repeats:YES];
}
